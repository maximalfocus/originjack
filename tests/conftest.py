...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    # Each pass configures the vulnerable API differently, and only one pass is live at a
    # time. A test written for another pass is skipped rather than run against the wrong
    # configuration.
    live_pass = (
        os.environ.get("ORIGINJACK_PASS_LABEL", "").strip()
        or os.environ.get("ORIGINJACK_VULNERABLE_SHAPE", "reflect").strip()
        or "reflect"
    )
    ...
    for item in items:
        ...
        pass_marker = item.get_closest_marker("demo_pass")
        if pass_marker and pass_marker.args and pass_marker.args[0] != live_pass:
            item.add_marker(
                pytest.mark.skip(
                    reason=f"pass {pass_marker.args[0]!r} is not the live pass ({live_pass!r})"
                )
            )
